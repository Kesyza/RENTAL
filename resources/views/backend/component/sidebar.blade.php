...
<ul class="navbar-nav bg-gradient-danger sidebar sidebar-dark accordion" id="accordionSidebar">
    ...
    <li class="nav-item">
        ...
        <div id="car" class="collapse {{active(['car.index','manufacture.index'],'show')}}" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
            ...
            <a class="collapse-item {{active('car.index')}}" href="{{route('car.index')}}">Data Mobil</a>
            <a class="collapse-item {{active('manufacture.index')}}" href="{{route('manufacture.index')}}">Data Merk</a>
            </div>
        </div>
    </li>
    ...
    <li class="nav-item">
        <a class="nav-link {{active('transaction.*','collapsed')}}" href="#" data-toggle="collapse" data-target="#transaksi" aria-expanded="true" aria-controls="transaksi">
            ...
            <span>Sewa Offline</span>
        </a>
        <div id="transaksi" class="collapse {{active('transaction.*','show')}}" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
            <a class="collapse-item {{active('transaction.create')}}" href="{{route('transaction.create')}}">Sewa</a>
            <a class="collapse-item {{active('transaction.index')}}" href="{{route('transaction.index')}}">List Sewa</a>
            <a class="collapse-item {{active('transaction.history')}}" href="{{route('transaction.history')}}">Riwayat Sewa</a>
            </div>
        </div>
    </li>
